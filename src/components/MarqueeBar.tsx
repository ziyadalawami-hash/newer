  ...
  return (
    <>
      <nav className={`fixed md:top-0 bottom-0 md:bottom-auto left-0 right-0 z-50 transition-colors duration-300 ${scrolled ? 'bg-[#000a01]/95 shadow-lg' : 'bg-[#000a01]'} md:shadow-none`}>
        <div className="max-w-[1000px] mx-auto bg-black">
      ...
      <div className="fixed md:top-0 bottom-0 md:bottom-auto left-0 md:left-1/2 md:-translate-x-1/2 z-50 w-full md:w-full md:max-w-[1000px] pointer-events-none" style={{ direction: 'ltr' }}>
        <div className="flex md:justify-end justify-between relative px-0">
          <div className="md:hidden relative group pointer-events-auto">
            <button
              className="flex items-center gap-2 bg-black text-white/90 hover:text-[#1fea00] transition-colors px-4 h-14 rounded-tr-lg"
              aria-label="Navigation menu"
            >
              <Menu size={18} />
            </button>

            <div className="absolute left-0 bottom-full bg-black border border-[#1c6000]/30 rounded-tl-lg shadow-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200">
              {navLinks.map((link) => (
                <a
                  key={link.href}
                  href={link.href}
          ...
          <div className="relative group pointer-events-auto">
            <button
              className="flex items-center gap-2 bg-black text-white/90 hover:text-[#1fea00] transition-colors px-4 h-14 md:h-10 md:rounded-none rounded-tl-lg"
